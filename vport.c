...
int main(int argc, char const *argv[])
{
    ...
    // wait for up forwarder and down forwarder
...
/**
 * init VPort intance: create TAP device and client socket
 */
...
void *forward_ether_data_to_tap(void *raw_vport)
{
    ...
    while (true)
    {
        ...
        int ether_datasz = recvfrom(vport->vport_sockfd, ether_data, sizeof(ether_data), 0, (struct sockaddr *)&vport->vswitch_addr, &vswitch_addr);
        if (ether_datasz > 0)
        {
            assert(ether_datasz >= 14);
            const struct ether_header *hdr = (const struct ether_header *)ether_data;
